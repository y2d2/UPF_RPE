...
from upf_rte.DataLoggers.TargetTrackingParticle_DataLogger import UKFTargetTrackingParticle_DataLogger
# Have to use this since Spyder_WS is a project.
from upf_rte.ParticleFilter.ConnectedAgentClass import UPFConnectedAgent
from upf_rte.ParticleFilter.TargetTrackingParticle import ListOfUKFLOSTargetTrackingParticles
from upf_rte.DataLoggers.ConnectedAgent_DataLogger import UPFConnectedAgentDataLogger
from upf_rte.ParticleFilter.TargetTrackingParticle import UKFLOSTargetTrackingParticle
from upf_rte.Simulation.BiRobotMovement import random_movements_host_random_movements_connected, \
    drone_flight, run_simulation, fix_host_fix_connected
from upf_rte.UtilityCode.utility_fuctions import get_4d_rot_matrix, cartesianToSpherical, inv_transformation_matrix, transform_matrix, get_states_of_transform
from upf_rte.Simulation.MultiRobotClass import MultiRobotSingleSimulation
...
class TestConnectedAgent(unittest.TestCase):
    ...
    def test_tc2(self):
        ...
        self.ca = UPFConnectedAgent([], x_ha_0=np.concatenate((self.host.x_start, [self.host.h_start])), id="0x000",
                                    particle_type=UKFLOSTargetTrackingParticle, drift_correction_bool=True,
                                    resample_factor=0.1)
        self.ca.set_ukf_parameters(kappa=-1, alpha=1, beta=2)
        self.ca.split_sphere_in_equal_areas(self.startMeasurement[0], 2*self.sigma_uwb,
                                            n_altitude=3, n_azimuth=4, n_heading=4)
        # self.ca.resample = self.ca.branch_kill_resampling
        self.dl = UPFConnectedAgentDataLogger(self.host, self.drone, self.ca, particle_type=UKFTargetTrackingParticle_DataLogger)
        self.dl.log_data(0)
        self.run_test(nlos_function=None)
