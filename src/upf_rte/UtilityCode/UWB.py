...
import numpy as np
from upf_rte import UtilityCode as TMF
import upf_rte.UtilityCode.turtlebot4 as TB
...
class UWB:
    ...
    def plot_real(self, factor=1, ax = None):
        ...
        ax.plot(t,[d for i, d in enumerate(np.abs(er)) if (i % factor == 0)], '-', color="tab:purple", label=r"Error $\epsilon_{d} = |d - \tilde{d} |$ [m]", linewidth=2)
        ax.grid()
