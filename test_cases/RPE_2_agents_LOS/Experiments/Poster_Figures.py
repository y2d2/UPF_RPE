...
def boxplots(df, methods_names, methods_colors, methods_legends):
    ...
    try:
        ...
        g.axes_dict["calculation_time"].set_ylabel(taa.y_label["calculation_time"])
    sns.move_legend(g, loc="upper center", bbox_to_anchor=(0.5, 0.98), ncol=4)
    plt.tight_layout()
    plt.suptitle("Boxplots over all experiments")
...
def lineplots(df, methods_names, methods_colors, methods_legends):
    g = taa.lineplot(df, methods_names, methods_colors,
                     methods_legends=methods_legends, variables=["calculation_time"],)
    try:
        g[0].set_ylabel(taa.y_label["calculation_time"])
        # g[1].set_ylabel(taa.y_label["error_h_relative"])
        g[0].set_xlabel("time [s]")
        # g[1].set_xlabel("time [s]")
    except:
        plt.ylabel(taa.y_label["error_x_relative"])
        plt.xlabel("Time [s]")
    ...
    fig = plt.gcf()
    fig.legend(handles=legend_handles, labels=legend_labels, ncol=2, loc="upper center",
               bbox_to_anchor=(0.5, 0.92))
    plt.tight_layout()
    plt.suptitle("Average error evolution over 5 experiments")
...
methods_names = [
    ...
    "QCQP|frequency=10.0|horizon=100|Type_['experiment']|Variable_['calculation_time']|Sigma_dv_[0.08]|Sigma_dw_[0.08]|Sigma_uwb_[0.25]|Frequency_[10.0]",
    ...
    "losupf|frequency=10.0|resample_factor=0.1|sigma_uwb_factor=1.0|Type_['experiment']|Variable_['calculation_time']|Sigma_dv_[0.08]|Sigma_dw_[0.08]|Sigma_uwb_[0.25]|Frequency_[10.0]",
]

methods_colors["QCQP|frequency=10.0|horizon=100|Type_['experiment']|Variable_['error_x_relative', 'error_h_relative', calculation_time]|Sigma_dv_[0.08]|Sigma_dw_[0.08]|Sigma_uwb_[0.25]|Frequency_[10.0]"] = "tab:blue"
